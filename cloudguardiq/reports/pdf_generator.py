...
class ComplianceReportGenerator:
    ...
    async def generate(
        ...
        *,
        ...
        include_policy_findings = any(self._is_azure_policy_finding(f) for f in relevant)
        ...
        story += self._scope_and_methodology(
            ...
            report_id=report_id,
            include_policy_findings=include_policy_findings,
        )
        story.append(PageBreak())
    ...
    def _scope_and_methodology(
        ...
        *,
        ...
        if include_policy_findings:
            methodology += (
                " This report includes Azure Policy Regulatory Compliance findings "
                "(rule IDs prefixed with AZPOL-) with per-control evidence from "
                "Microsoft's authoritative policy evaluation."
            )
            ...
            [
                ...
                (
                    ...
                    + (
                        ...
                        if include_policy_findings
                        else "CloudGuardIQ native compliance rules; "
                    )
                    + "optional Defender for Cloud posture"
                ),
            ],
    ...
    def _critical_findings(self, top: list[FindingResult]) -> list:
        ...
        for f in top:
            ...
            data.extend(self._policy_detail_rows(f))
    ...
    def _appendix(self, relevant: list[FindingResult]) -> list:
        ...
        for f in relevant:
            ...
            for detail in self._policy_detail_lines(f):
    ...
    @staticmethod
    def _is_azure_policy_finding(f: FindingResult) -> bool:
        return (f.rule_id or "").upper().startswith("AZPOL-")

    @classmethod
    def _policy_detail_rows(cls, f: FindingResult) -> list[list[str]]:
        if not cls._is_azure_policy_finding(f):
            return []
        evidence = f.evidence or {}
        rows: list[list[str]] = [["Finding source", "Azure Policy Regulatory Compliance"]]
        policy_definition_id = evidence.get("policy_definition_id")
        if policy_definition_id:
            rows.append(["Policy definition", cls._safe(str(policy_definition_id))])
        policy_assignment_id = evidence.get("policy_assignment_id")
        if policy_assignment_id:
            rows.append(["Policy assignment", cls._safe(str(policy_assignment_id))])
        reason_code = evidence.get("compliance_reason_code")
        if reason_code:
            rows.append(["Compliance reason", cls._safe(str(reason_code))])
        timestamp = evidence.get("timestamp")
        if timestamp:
            rows.append(["Policy evaluated at", cls._safe(str(timestamp))])
        return rows

    @classmethod
    def _policy_detail_lines(cls, f: FindingResult) -> list[str]:
        if not cls._is_azure_policy_finding(f):
            return []
        evidence = f.evidence or {}
        lines: list[str] = [
            "Source: Azure Policy Regulatory Compliance (AZPOL).",
        ]
        policy_definition_id = evidence.get("policy_definition_id")
        if policy_definition_id:
            lines.append(
                "Policy definition: "
                f"<font name='Courier'>{cls._safe(str(policy_definition_id))}</font>"
            )
        policy_assignment_id = evidence.get("policy_assignment_id")
        if policy_assignment_id:
            lines.append(
                "Policy assignment: "
                f"<font name='Courier'>{cls._safe(str(policy_assignment_id))}</font>"
            )
        reason_code = evidence.get("compliance_reason_code")
        if reason_code:
            lines.append(f"Compliance reason: {cls._safe(str(reason_code))}")
        timestamp = evidence.get("timestamp")
        if timestamp:
            lines.append(f"Evaluated at: {cls._safe(str(timestamp))}")
        return lines
    @staticmethod
